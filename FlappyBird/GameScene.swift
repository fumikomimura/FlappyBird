...
var itemNode:SKNode!  //❤️課題用　追加　いらない？
...
//let itemCategory: UInt32 = 1 << 4     //❤️ 課題　追加　いらない？
...
//音を追加するためのメソッド
var sound = SKAction.playSoundFileNamed("itemget", waitForCompletion: false)
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func restart() {
        ...
        itemNode.removeAllChildren()   //❤️課題用追加
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            playSound(sound: sound)   //アイテム取得した時に音が鳴る
    ...
    func setupItem() {
        ...
        let createItemAnimation = SKAction.run({
            // 壁関連のノードを乗せるノードを作成
            let item = SKNode()
            let random_y_range = itemTexture.size().height * 10 //追加
            // 0〜random_y_rangeまでのランダム値を生成
            let random_y = CGFloat.random(in: 0..<random_y_range)
            
            item.position = CGPoint(x: self.frame.size.width + itemTexture.size().width / 2, y: 0)  //アイテムの出現場所の座標　xを150にした
            item.zPosition = 10 // 雲より手前、地面より奥　　zposition＝画像の重ねかた
            
            let subItem = SKSpriteNode(texture: itemTexture)
            subItem.position = CGPoint(x: 0, y: random_y)
            ...
            itemScoreNode.physicsBody?.categoryBitMask = itemScoreCategory
            itemScoreNode.physicsBody?.contactTestBitMask = birdCategory
